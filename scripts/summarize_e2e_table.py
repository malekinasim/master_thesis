...
def find_model_dirs(out_root: Path) -> list[Path]:
    ...
    if not reports_dir.exists():
        return []
    return sorted(
        [p for p in reports_dir.iterdir() if p.is_dir() and p.name not in {"figures", "tables"}]
    )


def read_e2e_summary(
    out_root: Path,
    model_dir_name: str,
    task: str
) -> dict | None:
    ...
    auc_csv = tables_dir / f"{task}_e2e_robustness_per_layer.csv"
    acc_csv = tables_dir / f"{task}_e2e_noise_curves.csv"
    ...
    # baseline accuracy at sigma = 0 on the last layer
...
def build_summary_table(out_root: Path, models: list[str] | None = None) -> pd.DataFrame:
    ...
    for model_dir_name in model_dirs:
        for task in TASKS:
            row = read_e2e_summary(out_root, model_dir_name, task)
            if row is not None:
                rows.append(row)

    if not rows:
        raise FileNotFoundError("No valid end-to-end CSV pairs found under out/reports/...")

    df = pd.DataFrame(rows)

    task_order = {"MCQ": 0, "SINGLE": 1}
    df["_task_order"] = df["Task"].map(task_order)
    df = df.sort_values(["_task_order", "Model"]).drop(columns=["_task_order"])
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build summary table for end-to-end robustness.")
    ...
    ap.add_argument(
        "--out_csv",
        default=None,
        help="Default: <out_root>/reports/tables/table_e2e_robustness_summary.csv",
    )
    args = ap.parse_args()

    out_root = Path(args.out_root)
    print(f"out rout{out_root}")
    out_csv = (
        Path(args.out_csv)
        if args.out_csv
        else out_root / "reports" / "tables" / "table_e2e_robustness_summary.csv"
    )
